...
class Fitbit(object):
    ...
    def log_weight(self,data):
        """
        https://dev.fitbit.com/docs/body/#log-weight
        """
...
## Andrew's code
    def get_food(self,date):
		...
		https://dev.fitbit.com/docs/devices/#get-devices
        """
        url = "{0}/{1}/user/-/foods/log/date/{timestamp}.json".format(*self._get_common_args(),timestamp=date.strftime("%Y-%m-%d"))
        return self.make_request(url)
    def get_heart(self,date,period = '1d'):
        url = "{0}/{1}/user/-/activities/heart/date/{date}/{period}.json".format(
            *self._get_common_args(),
    ...
    def get_hrv(self,date):
        url = 	"{0}/{1}/user/-/hrv/date/{date}.json".format(*self._get_common_args(),date=date.strftime("%Y-%m-%d"))
        return self.make_request(url)
    def get_activity(self,**query_params):
        '''
        Very fucking in depth response, overkill - don't use. Jesus Christ fair play like but who's using this
        '''
        assert 'afterDate' in query_params or 'beforeDate' in query_params, 'Either afterDate or beforeDate must be passed as query parameter. Also must be timelike object.'
        if 'afterDate' in query_params: query_params['afterDate'] = query_params['afterDate'].strftime('%Y-%m-%d') 
        if 'beforeDate' in query_params: query_params['beforeDate'] = query_params['beforeDate'].strftime('%Y-%m-%d')

        if 'sort' not in query_params: query_params['sort'] = 'asc'
        if 'offset' not in query_params: query_params['offset'] = 0
        if 'limit' not in query_params: query_params['limit'] = 100
        
        query_params = f'{"?" if query_params else ''}'+ '&'.join([f'{k}={v}' for k,v in query_params.items()])
        url = "{0}/{1}/user/-/activities/list.json".format(*self._get_common_args()) + query_params
        return self.make_request(url)

    def get_DailyActivitySummary(self,date):
        date = date.strftime('%Y-%m-%d')
        url = "{0}/{1}/user/-/activities/date/{date}.json".format(*self._get_common_args(),date=date)
        return self.make_request(url)
    def get_bodyweight(self,date):
        url = "{0}/{1}/user/-/body/log/weight/date/{date}.json".format(*self._get_common_args(),date=date.strftime('%Y-%m-%d'))
        return self.make_request(url)